...
export default function ValvulasPage() {
  ...
  const paginatedData = useMemo(() => {
    ...
    return filteredAndSortedValvulas.slice(start, start + itemsPerPage);
  }, [filteredAndSortedValvulas, currentPage]);

  const totalPages = Math.ceil(filteredAndSortedValvulas.length / itemsPerPage);
    ...
    <div className={styles.container}>
      ...
      <div className={styles.filtersRow}>
        ...
        <div className={styles.resultsCount}>
          Mostrando {paginatedData.length} de <strong>{filteredAndSortedValvulas.length}</strong> activos
        </div>
      </div>
      ...
      <div className="glass" style={{ marginTop: '2rem', overflow: 'hidden' }}>
        ...
        {paginatedData.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>
            No hay válvulas registradas que coincidan con la búsqueda.
          </div>
        )}
