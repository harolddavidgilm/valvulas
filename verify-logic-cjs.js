const { calculateASMETolerance, getRiskLevel, getRecommendedInterval } = require('./src/lib/valve-logic');
...
function testTolerance() {
  ...
  cases.forEach(({ p, n, expected }) => {
    ...
    console.log(`P=${p}, Norm=${n} | Expected: ${expected} | Actual: ${actualText} | ${actualText === expected ? '✅' : '❌'}`);
  });
}
